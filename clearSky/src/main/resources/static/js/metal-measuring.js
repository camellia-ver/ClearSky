    ...
    new Chart(ctx, {
        ...
        options: {
            ...
            scales: {
                ...
                y: {
                    type: 'logarithmic',
                    title: { display: true, text: '농도 (µg/m³) (로그 스케일)' },
                    min: 0.001
                }
            },
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        label: function (context) {
                            const metal = context.dataset.label;
            ...
            <div class="card h-100 shadow-sm border">
                <div class="card-body">
                    ...
                    <p class="card-text mb-1">
                        <span class="text-muted">기준치:</span> <strong>${threshold}</strong> µg/m³
                    </p>
                    <p class="card-text mb-1">
                        최고: <strong>${maxRegion}</strong> (${maxValue} µg/m³) ${riskMax}
                    </p>
                    <p class="card-text mb-1">
                        최저: <strong>${minRegion}</strong> (${minValue} µg/m³) ${riskMin}
                    </p>
                    <p class="card-text text-truncate">
                        <span class="fw-semibold">건강 영향:</span> ${healthImpact}
                    </p>
                </div>
            </div>
        `;
